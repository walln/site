...
Let <LatexInLine formula="f_T : \mathcal{X} \to \mathbb{R}^{d_T}" /> be the teacher encoder and <LatexInLine formula="f_S : \mathcal{X} \to \mathbb{R}^{d_S}" /> the student. During Stage A we minimise

<Latex formula="\mathcal{L}(W) = \mathbb{E}_{x \sim \mathcal{D}} \bigl\| f_T(x) - W f_S(x) \bigr\|_2^{2} " />

where <LatexInLine formula="W \in \mathbb{R}^{d_T \times d_S}" /> is an (often implicit) linear projection that aligns the two representation spaces (it is learned directly when using cosine/MSE losses). **We further assume the embeddings are mean-centred; otherwise one can subtract the dataset mean or learn an additional bias term.** The closed-form optimum is the whitened Procrustes solution

<Latex formula="W^{\star} = \Sigma_{TS} \, \Sigma_{SS}^{-1}" />

with cross-covariance <LatexInLine formula="\Sigma_{TS} = \mathbb{E}[f_T(x) f_S(x)^{\top}]" /> and student covariance <LatexInLine formula="\Sigma_{SS} = \mathbb{E}[f_S(x) f_S(x)^{\top}]" />. Thus, after distillation we have approximately

<Latex formula="f_T(x) \; \approx \; W^{\star} \, f_S(x)" />
...
<LatexInLine formula="J_{f_T}(x) \approx W^{\star} \, J_{f_S}(x)" />, *so their tangent spaces are aligned by* <LatexInLine formula="W^{\star}" /> and first-order geometry is preserved. Stage B therefore only needs to model the small, higher-order residuals that remain.
