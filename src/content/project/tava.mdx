...
<LatexInLine formula="J_{f_T}(x) \approx W^{\star} \, J_{f_S}(x)" />, *so their
tangent spaces are aligned by* <LatexInLine formula="W^{\star}" /> and
first-order geometry is preserved. Stage B therefore only needs to model the
small, higher-order residuals that remain.
